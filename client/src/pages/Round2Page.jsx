...
import { Bug, CheckCircle2, XCircle, ArrowRight, Trophy, AlertTriangle, Sparkles, Layers } from 'lucide-react';
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await api.submitR2Answer({
        ...
        selected_bug_type: selectedBugType
      });
        ...
        <div className="max-w-xl w-full p-8 rounded-2xl border border-amber-400/40 bg-slate-900/90 backdrop-blur text-center shadow-2xl">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="flex-1 py-3 rounded-xl border border-amber-400/50 bg-amber-500/10 text-amber-300 font-mono font-bold text-xs hover:bg-amber-500/20 transition flex items-center justify-center gap-2"
            >
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto p-4 md:p-6 flex flex-col">
        ...
        <div className="flex items-center justify-between mb-4 font-mono text-xs">
          ...
          <div className="text-[11px] text-slate-400">
            Scoring: Correct Line = <span className="text-cyber-green font-bold">3 Marks</span> • Bug Type = <span className="text-cyber-cyan font-bold">2 Marks</span>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-950 overflow-hidden shadow-2xl mb-6">
          <div className="px-4 py-2 bg-slate-900/80 border-b border-slate-800 flex items-center justify-between text-xs font-mono text-slate-400">
            <span>Java Source Code (Click a line or choose below)</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="p-5 rounded-xl border border-slate-800 bg-slate-900/80">
            <label className="block text-xs font-mono font-bold text-cyber-cyan uppercase mb-2">
              1. Which line contains the primary bug? (3 Marks)
            </label>
          ...
          <div className="p-5 rounded-xl border border-slate-800 bg-slate-900/80">
            <label className="block text-xs font-mono font-bold text-amber-400 uppercase mb-2">
              2. What type of bug is present? (2 Marks)
            </label>
          ...
          <div className={`p-4 rounded-xl border font-mono text-xs mb-6 ${
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-3 text-[11px]">
                <span>Line: {feedback.line_correct ? '✓ Correct (3pts)' : `✗ Faulty Line was ${feedback.actual_buggy_line}`}</span>
                <span>Type: {feedback.type_correct ? '✓ Correct (2pts)' : `✗ Type was ${feedback.actual_bug_type}`}</span>
              </div>
            </div>
        ...
        <div className="flex justify-end mt-auto">
          {!isSubmitted ? (
            <button
              ...
              className="px-8 py-3.5 rounded-xl bg-amber-400 text-slate-950 font-bold font-mono text-xs uppercase tracking-wider hover:bg-amber-300 transition disabled:opacity-40 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              className="px-8 py-3.5 rounded-xl bg-gradient-to-r from-cyber-green to-emerald-500 text-slate-950 font-bold font-mono text-xs uppercase tracking-wider hover:opacity-90 transition flex items-center gap-2"
            >
